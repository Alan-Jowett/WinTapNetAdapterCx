fn main() -> Result<(), wdk_build::ConfigError> {
    ...
    let wdk_root = std::env::var("WDKContentRoot")
        .expect("WDKContentRoot must identify the pinned WDK content root");
    ...
    let netadapter_lib = std::path::Path::new(&wdk_root)
        .join("Lib")
        .join("10.0.28000.0")
        .join("km")
        .join(lib_arch)
        ...
        .join("adapter")
        .join("2.5");
    println!("cargo:rustc-link-search=native={}", netadapter_lib.display());
