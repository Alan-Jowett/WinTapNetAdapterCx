...
| TC-085 | Run the switch with both dynamic endpoints while forcing transient `ERROR_BUSY` I/O-ring completions. Verify bounded backoff retries the same operation without slot reuse, frame loss caused by premature buffer release, stale completions, or unbounded looping; verify fatal completion errors remain surfaced. |
