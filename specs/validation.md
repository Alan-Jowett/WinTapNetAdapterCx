...
| VAL-023 | REQ-025 | Verify passive READ delivery claims frame/request ownership under the capture queue's queue-local lock, never the lifecycle/control lock, and performs WDF buffer access, copying, requeue, and completion only after releasing it; verify no duplicate claims across packet callbacks, `evt_io_read`, and the work item, including too-small-buffer, cancellation, stop, and teardown races. |
...
| VAL-042 | REQ-050 | Instrument simultaneous TX and RX packet-queue advancement. Verify neither callback waits for a driver lock held by the opposite direction and neither blocks on a lock a teardown path can hold; verify queue-local contention, atomic adaptive wait races, exact-once unmark/cancel handoff, retirement of the wait record before completion, queue-stop ownership, and nested callback-lifetime lease quiescence preserve ring ownership, owner-generation isolation, and exactly-once completion. |
...
| TC-066 | Instrument `evt_io_read` and passive completion paths while a captured frame and READ request are available; verify ownership is claimed under the capture queue's queue-local lock and not the lifecycle/control lock, the lock is released before WDF buffer retrieval/copy/completion, too-small or failed retrieval requeues the frame under that same lock and only for the owner observed under the requeue's callback-lifetime lease, and concurrent callback/work-item/cancellation/teardown races complete exactly once. |
...
| TC-097 | Race readable and writable queue transitions against `WAIT_FOR_CHANGE` registration, including TX-only and RX-only packet-queue stops immediately before registration, between the `REGISTERING` claim and request-slot publication, after slot publication, and before and after `WdfRequestMarkCancelableEx`, plus cancellation, owner cleanup, queue close, and release hardware while TX and RX advances run concurrently. Verify the control queue's automatic synchronization serializes its request handlers with `EvtIoStop` at `PASSIVE_LEVEL`, while concurrent packet-queue stop is detected by the requested-direction started-state recheck or atomically claims the published `REGISTERING` record; a request is not transition-visible before successful marking and no wait becomes pending for a requested direction that stopped during registration; a `STATUS_CANCELLED` or other failing mark return clears registration and is completed by registration because WDF does not invoke the cancellation callback; a queue stop completes only a request its atomic claim exclusively owns and otherwise neither acknowledges nor completes, while the losing claim's owner still completes within the power-transition budget; no callback holding the queue synchronization lock blocks on a driver lock a teardown path holds; the wait state, satisfied mask, and registration sequence occupy one atomic word so the passive worker never observes a scheduled wait with an unpublished mask, concurrent transitions accumulate their bits, and a stalled claim cannot leak into or schedule a later registration; generation/recheck handling has no lost wakeup; a packet-queue advance only makes the atomic claim and schedules passive work, never unmarking or completing; exactly one path calls `WdfRequestUnmarkCancelable` per marked request and terminally completes each wait; every resolved terminal owner retires the wait record and clears the published request slot before completing, so a recycled request handle never matches the slot, while the unresolved `STATUS_CANCELLED` handoff and the cancellation of a `REGISTERING`/teardown record keep ownership metadata published until resolution; a `STATUS_CANCELLED` unmark result is resolved by the cancel handoff so exactly one of the claimant and the cancellation callback completes the request and no unmark occurs after the cancellation callback completed it; an unexpected unmark failure terminally completes the request and retires the wait so a later wait is not rejected as busy; a cancelled `REGISTERING`/teardown handoff never leaves the wait state stuck; and neither advance waits for adaptive-wait synchronization. |
| TC-098 | Start concurrent TX/RX advances and passive READ/capture-drain work, then exercise queue stop, RX cancel, owner cleanup, D0 exit, surprise removal, and release hardware at every published-metadata boundary, including power or hardware transitions that begin immediately before and during owner cleanup. Verify framework quiescence or callback-lifetime leases prevent stale ring, extension, queue, and lookaside access; verify admission closure and lease acquisition are atomic so no callback is admitted after admission closes, lease acquisition never blocks, both directions are quiesced, and teardown holds no lock a leased callback can wait on while draining; verify passive READ and capture-drain paths hold the capture lease from their owner/lifecycle snapshot through dequeue, delivery, and any requeue; verify quiescence nests, so owner cleanup cannot readmit callbacks while D0 exit or release hardware remains quiesced, an early return from owner cleanup releases only its own closure, readmission never underflows or readmits a scope that did not close admission, and owner cleanup claims an owner-specific closing state only from `OPEN`, preventing it from publishing `OPEN` or resuming the manual queue when a power or hardware transition owns lifecycle before or during cleanup; verify a retired owner's captured or dequeued frames are released rather than delivered, enqueued, or requeued for the next owner; ring ownership/index rules remain intact; and frame/request cleanup is exact-once. |
