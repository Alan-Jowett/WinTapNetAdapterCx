...
| VAL-038 | REQ-047 | Verify per-endpoint adaptive negotiation diagnostics, periodic idle wait statistics, and a GUID-correlated two-TAP probe. Reject stale TAP interfaces; prove enable, wait registration, readable capture, wait completion, resumed read, and peer forwarding before recording performance data. |
