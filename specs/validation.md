...
| TC-098 | Start concurrent TX/RX advances, then exercise queue stop, RX cancel, owner cleanup, D0 exit, surprise removal, and release hardware at every published-metadata boundary, including overlapping owner cleanup with D0 exit and with release hardware. Verify framework quiescence or callback-lifetime leases prevent stale ring, extension, queue, and lookaside access; verify admission closure and lease acquisition are atomic so no callback is admitted after admission closes, lease acquisition never blocks, both directions are quiesced, and teardown holds no lock a leased callback can wait on while draining; verify quiescence nests, so owner cleanup cannot readmit callbacks while D0 exit or release hardware remains quiesced, an early return from owner cleanup releases only its own closure, and readmission never underflows or readmits a scope that did not close admission; verify a retired owner's captured or dequeued frames are released rather than enqueued or requeued into the next owner's queues; ring ownership/index rules remain intact; and frame/request cleanup is exact-once. |
